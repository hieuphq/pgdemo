...
Pgdemo is demostration for [pg](https://www.erlang.org/doc/man/pg.html).
...
We start 2 instances of a server with a specific name, connect them together and publish the message from an instance. We can see the message that is broadcast to another.
...
The use-case can be when a new child joins the cluster, we can synch data from the old ones.
